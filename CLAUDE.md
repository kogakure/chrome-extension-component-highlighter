...
**`options.js`** — options page script. Auto-saves all keys on change (debounced). Manages `highlightColor`, `outlineStyle`, `outlineWidth`, `dataAttribute`, `customCSS`. Supports import/export settings as JSON.
...
| `highlightColor`        | string (hex)                          | `"#3b82f6"`        | Outline/badge color; injected via `<style data-ch-color>` |
| `outlineStyle`          | `"solid"` \| `"dashed"` \| `"dotted"` | `"solid"`          | CSS `outline-style`; injected via `<style data-ch-color>` |
| `outlineWidth`          | number (1–4)                          | `2`                | CSS `outline-width` in px; injected via `<style data-ch-color>` |
